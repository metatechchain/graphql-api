...
type DefiToken {
    ...
    # canWrapMTC signals if the token can be used
    # to wrap native MTC tokens for DeFi trading.
    canWrapMTC: Boolean!
...
# Transaction is an MetaTech block chain transaction.
type Transaction {
...
# Block is an MetaTech block chain block.
type Block {
...
# Delegation represents a delegation on MetaTech block chain.
type Delegation {
    ...
    # outstandingSMTC represents the amount of sMTC tokens representing
    # the tokenized stake minted and un-repaid on this delegation.
    outstandingSMTC: BigInt!

    # tokenizerAllowedToWithdraw indicates if the stake tokenizer allows the stake
    # to be withdrawn. That means all the sMTC tokens have been repaid and the sMTC
...
# Address is a 20 byte MetaTech address, represented as 0x prefixed hexadecimal number.
scalar Address
...
# MtcBlockBurn represents a native MTC tokens burn record per created block.
type MtcBlockBurn {
    ...
    # amount represents the amount of MTC tokens burned in WEI units (18 digits fixed INT encoded as HEX number).
    amount: BigInt!

    # mtcValue represents MTC value of the burned MTC tokens.
    mtcValue: Float!
}
...
# Price represents price information of core MetaTech token
type Price {
    "Source unit symbol."
    fromSymbol: String!
...
type Query {
    ...
    # Total number of accounts active on the MetaTech blockchain.
    accountsActive:Long!
    ...
    # Get the id of the current epoch of the MetaTech blockchain.
    currentEpoch:Long!
    ...
    # The last staker id in MetaTech blockchain.
    lastStakerId: Long!

    # The number of stakers in MetaTech blockchain.
    stakersNum: Long!
    ...
    # Get price details of the MetaTech blockchain token for the given target symbols.
    price(to:String!):Price!

    # Get calculated staking rewards for an account or given
    # staking amount in MTC tokens.
    ...
    # by the Uniswap Core contract on MetaTech blockchain.
    ...
    # mtcBurnedTotal provides the total amount of native MTC tokens burned
    # by the chain from paid transaction fees in WEI units.
    mtcBurnedTotal: BigInt!

    # mtcBurnedTotalAmount provides the total amount of native MTC tokens burned
    # by the chain from paid transaction fees in MTC units.
    mtcBurnedTotalAmount: Float!

    # mtcLatestBlockBurnList provides a list of latest burned native MTC tokens per-block.
    mtcLatestBlockBurnList(count: Int = 25): [MtcBlockBurn!]!
}
