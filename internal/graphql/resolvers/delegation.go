...
// OutstandingSMTC resolves the amount of outstanding sMTC tokens
// minted for this account.
func (del Delegation) OutstandingSMTC() (hexutil.Big, error) {
	val, err := repository.R().DelegationOutstandingSMTC(&del.Address, del.Delegation.ToStakerId)
	if err != nil {
		return hexutil.Big{}, err
	}
	return *val, nil
