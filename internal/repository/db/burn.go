...
// colBurns represents the name of the native MTC burns collection in database.
...
// StoreBurn stores the given native MTC burn record.
func (db *MongoDbBridge) StoreBurn(burn *types.MtcBurn) error {
	if burn == nil {
		return nil
	}
	...
	if sr.Err() != nil {
		...
		db.log.Errorf("could not load MTC burn at #%d; %s", burn.BlockNumber, sr.Err())
		return sr.Err()
	}

	// decode existing burn and update
	var ex types.MtcBurn
	if err := sr.Decode(&sr); err != nil {
		db.log.Errorf("could not decode MTC burn at #%d; %s", burn.BlockNumber, sr.Err())
		return sr.Err()
	}
...
// BurnList provides list of native MTC burns per blocks stored in the persistent database.
func (db *MongoDbBridge) BurnList(count int64) ([]types.MtcBurn, error) {
	col := db.client.Database(db.dbName).Collection(colBurns)
	...
	list := make([]types.MtcBurn, 0, count)

	for cr.Next(ctx) {
		var row types.MtcBurn
		if err := cr.Decode(&row); err != nil {
