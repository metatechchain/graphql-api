...
import (
	...
	"metatech-api-graphql/internal/repository/db"
...
func connect(cfg *config.Config, log logger.Logger) (*cache.MemBridge, *db.MongoDbBridge, *rpc.MtcBridge, error) {
	...
	// create new Meta RPC bridge
	rpcBridge, err := rpc.New(cfg, log)
	if err != nil {
		log.Criticalf("can not connect Meta RPC interface, %s", err.Error())
		return nil, nil, nil, err
	}
