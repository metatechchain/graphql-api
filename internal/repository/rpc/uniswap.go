...
func (mtc *MtcBridge) NativeTokenAddress() (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(mtc.uniswapConfig.Router, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap router contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("Native token address not available; %s", err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) UniswapPair(tokenA *common.Address, tokenB *common.Address) (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(mtc.uniswapConfig.Core, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap factory contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("Uniswap pair not found for tokens %s and %s; %s", tokenA.String(), tokenB.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) UniswapPairs(whiteListedOnly bool) ([]common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(mtc.uniswapConfig.Core, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap factory contract not found; %s", err.Error())
	...
	if err != nil || length == nil {
		mtc.log.Error("Uniswap pairs array length not available")
		return nil, err
	}
	...
	for i := uint64(0); i < length.Uint64(); i++ {
		...
		if err != nil {
			mtc.log.Errorf("error loading Uniswap pair; %s", err.Error())
			continue
		}

		// check the pair (is it on the white list?)
		// add the address to the list, if it's ok
		if !whiteListedOnly || mtc.isUniswapPairWhitelisted(&adr) {
...
func (mtc *MtcBridge) isUniswapPairWhitelisted(pair *common.Address) bool {
	// decode the pair address
	pairAddr := pair.String()

	// check the pair address against all the white listed pairs in config
	for _, addr := range mtc.uniswapConfig.PairsWhiteList {
...
func (mtc *MtcBridge) UniswapQuoteInput(
	amountA hexutil.Big,
	...
	if err != nil {
		mtc.log.Errorf("can not calculate Uniswap quote; %s", err.Error())
		return hexutil.Big{}, err
	}
...
func (mtc *MtcBridge) UniswapAmountsOut(amountIn hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(mtc.uniswapConfig.Router, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return mtc.uniswapLoadAmounts(contract.GetAmountsOut, (*big.Int)(&amountIn), tokens)
}
...
func (mtc *MtcBridge) UniswapAmountsIn(amountOut hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(mtc.uniswapConfig.Router, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return mtc.uniswapLoadAmounts(contract.GetAmountsIn, (*big.Int)(&amountOut), tokens)
}

// uniswapLoadAmounts loads a list of swap amounts using the given loader function
// of the Uniswap router contract.
func (mtc *MtcBridge) uniswapLoadAmounts(
	...
	if err != nil {
		mtc.log.Errorf("can not load swap amounts; %s", err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) UniswapTokens(pair *common.Address) ([]common.Address, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s token A not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s token B not found; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) UniswapReserves(pair *common.Address) ([]hexutil.Big, error) {
	// get the reserves record from the contract
	rs, err := mtc.uniswapReservesRecord(pair)
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) UniswapReservesTimeStamp(pair *common.Address) (hexutil.Uint64, error) {
	// get the reserves record from the contract
	rs, err := mtc.uniswapReservesRecord(pair)
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
...
func (mtc *MtcBridge) UniswapCumulativePrices(pair *common.Address) ([]hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s cumulative price A not available; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s cumulative price B not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) uniswapReservesRecord(pair *common.Address) (*struct {
	Reserve0           *big.Int
	Reserve1           *big.Int
...
func (mtc *MtcBridge) UniswapLastKValue(pair *common.Address) (hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		mtc.log.Errorf("Uniswap pair %s coefficient K not available; %s", pair.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (mtc *MtcBridge) UniswapPairContract(pairAddres *common.Address) (*contracts.UniswapPair, error) {
	contract, err := contracts.NewUniswapPair(*pairAddres, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap pair contract %s not found; %s", pairAddres.String(), err.Error())
		return nil, err
	}
	return contract, nil
}
...
func (mtc *MtcBridge) UniswapFactoryContract() (*contracts.UniswapFactory, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(mtc.uniswapConfig.Core, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Uniswap factory contract not found; %s", err.Error())
