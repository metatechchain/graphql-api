...
func (mtc *MtcBridge) FMintAccount(owner *common.Address) (*types.FMintAccount, error) {
	// make the container
	var err error
	da := types.FMintAccount{Address: *owner}

	// load list of collateral tokens
	tokenList, err := mtc.DefiTokenList()
	if err != nil {
		mtc.log.Errorf("collateral tokens list loader failed; %s", err.Error())
		return nil, err
	}

	// debt tokens are the same
	da.CollateralList = mtc.FMintFilterTokens(owner, tokenList, types.DefiTokenTypeCollateral)
	da.DebtList = mtc.FMintFilterTokens(owner, tokenList, types.DefiTokenTypeDebt)

	// get the current values of the account tokens on both collateral and debt
	da.CollateralValue, da.DebtValue, err = mtc.fMintAccountValue(*owner)
	if err != nil {
		mtc.log.Errorf("can not pull account tokens value; %s", err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) FMintFilterTokens(owner *common.Address, list []common.Address, tp types.DefiTokenType) []common.Address {
	zero := new(big.Int)
	result := make([]common.Address, 0)
	for _, adr := range list {
		val, err := mtc.FMintTokenBalance(owner, &adr, tp)
		if err != nil {
			mtc.log.Errorf("failed to get the token balance; %s", err.Error())
			continue
		}
...
func (mtc *MtcBridge) FMintPoolBalance(pool *contracts.DeFiTokenStorage, owner *common.Address, token *common.Address) (hexutil.Big, error) {
	// get the collateral token balance
	val, err := pool.BalanceOf(nil, *owner, *token)
	if err != nil {
		mtc.log.Debugf("pool balance failed on token %s, account %s; %s", token.String(), owner.String(), err.Error())
		return hexutil.Big{}, err
	}

	// do we have a value?
	if val == nil {
		mtc.log.Debugf("token %s balance not available for owner %s", token.String(), owner.String())
...
func (mtc *MtcBridge) FMintTokenBalance(owner *common.Address, token *common.Address, tp types.DefiTokenType) (hexutil.Big, error) {
	var err error
	...
	case types.DefiTokenTypeCollateral:
		pool, err = mtc.fMintCfg.fMintCollateralPool()
	case types.DefiTokenTypeDebt:
		pool, err = mtc.fMintCfg.fMintDebtPool()
	}

	// error in pool loading?
	if err != nil {
		mtc.log.Debugf("token storage pool failed to load; %s", err.Error())
		return hexutil.Big{}, err
	}

	// do we have the pool?
	if pool == nil {
		mtc.log.Debugf("token storage pool not available")
		return hexutil.Big{}, nil
	}

	return mtc.FMintPoolBalance(pool, owner, token)
}

// FMintTokenTotalBalance loads total balance of a single DeFi token by it's address.
func (mtc *MtcBridge) FMintTokenTotalBalance(token *common.Address, tp types.DefiTokenType) (hexutil.Big, error) {
	var err error
	var pool *contracts.DeFiTokenStorage
	...
	if err != nil {
		mtc.log.Debugf("pool total balance failed on token %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}

	// do we have a value?
	if val == nil {
		mtc.log.Debugf("token %s total balance not available", token.String())
		return hexutil.Big{}, nil
	}
...
func (mtc *MtcBridge) FMintTokenValue(owner *common.Address, token *common.Address, tp types.DefiTokenType) (hexutil.Big, error) {
	// get the balance
	balance, err := mtc.FMintTokenBalance(owner, token, tp)
	if err != nil {
		mtc.log.Errorf("token %s balance unknown; %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}

	// get the price for the given token from oracle
	val, err := mtc.FMintTokenPrice(token)
	if err != nil {
		mtc.log.Errorf("token %s price not available; %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (mtc *MtcBridge) FMintTokenPrice(token *common.Address) (hexutil.Big, error) {
	// get the price oracle address
	oracle, err := mtc.fMintCfg.priceOracleProxyContract()
	if err != nil {
		return hexutil.Big{}, err
	}
	...
	if err != nil {
		mtc.log.Errorf("price not available for token %s; %s", token.String(), err.Error())
		return hexutil.Big{}, nil
	}

	// do we have the value?
	if val == nil {
		mtc.log.Debugf("token %s has no value", token.String())
		return hexutil.Big{}, nil
	}
...
func (mtc *MtcBridge) fMintAccountValue(owner common.Address) (hexutil.Big, hexutil.Big, error) {
	...
	if err != nil {
		mtc.log.Errorf("joined collateral value loader failed")
		return hexutil.Big{}, hexutil.Big{}, err
	}
	...
	if err != nil {
		mtc.log.Errorf("joined debt value loader failed")
		return hexutil.Big{}, hexutil.Big{}, err
	}
...
func (mtc *MtcBridge) FMintRewardsEarned(addr *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := mtc.fMintCfg.fMintRewardsDistribution()
	if err != nil {
		return hexutil.Big{}, err
	}

	// get the block height
	bl, err := mtc.BlockHeight()
	if err != nil {
		return hexutil.Big{}, err
	}
	...
	if err != nil {
		mtc.log.Errorf("can not calculate earned rewards; %s", err.Error())
		return hexutil.Big{}, err
	}
...
// accumulated on the account for the excessive collateral deposits.
func (mtc *MtcBridge) FMintRewardsStashed(addr *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := mtc.fMintCfg.fMintRewardsDistribution()
	if err != nil {
		return hexutil.Big{}, err
	...
	if err != nil {
		mtc.log.Errorf("can not calculate stashed rewards; %s", err.Error())
		return hexutil.Big{}, err
	}
...
func (mtc *MtcBridge) FMintCanClaimRewards(addr *common.Address) (bool, error) {
	// connect the contract
	contract, err := mtc.fMintCfg.fMintMinterContract()
	if err != nil {
		mtc.log.Errorf("rewards claim check failed; %s", err.Error())
		return false, err
	}
	...
	if err != nil {
		mtc.log.Errorf("can not check rewards claim flag; %s", err.Error())
		return false, err
	}
...
func (mtc *MtcBridge) FMintCanReceiveRewards(addr *common.Address) (bool, error) {
	// connect the contract
	contract, err := mtc.fMintCfg.fMintMinterContract()
	if err != nil {
		mtc.log.Errorf("rewards eligibility check failed; %s", err.Error())
		return false, err
	}
	...
	if err != nil {
		mtc.log.Errorf("can not check rewards eligibility flag; %s", err.Error())
		return false, err
	}
...
func (mtc *MtcBridge) FMintCanPushRewards() (bool, error) {
	// connect the contract
	contract, err := mtc.fMintCfg.fMintRewardsDistribution()
	if err != nil {
		return false, err
	...
	if err != nil {
		mtc.log.Errorf("can not check rewards last push; %s", err.Error())
		return false, err
	}
