...
func (mtc *MtcBridge) ValidatorDowntime(valID *hexutil.Big) (uint64, uint64, error) {
	...
	if err := mtc.rpc.Call(&dt, "abft_getDowntime", valID); err != nil {
		mtc.log.Errorf("failed to get downtime of validator #%d; %s", valID.ToInt().Uint64(), err.Error())
		return 0, 0, err
	}
	// get the values
...
func (mtc *MtcBridge) ValidatorEpochUptime(valID *hexutil.Big) (uint64, error) {
	// use rather the public API, it should be faster since it does not involve contract call
	var ut hexutil.Uint64
	if err := mtc.rpc.Call(&ut, "abft_getEpochUptime", valID); err != nil {
		mtc.log.Errorf("failed to get epoch uptime of validator #%d; %s", valID.ToInt().Uint64(), err.Error())
		return 0, err
	}
	// get the values
...
// LastValidatorId returns the last staker id in MetaTech blockchain.
func (mtc *MtcBridge) LastValidatorId() (uint64, error) {
	// get the value from the contract
	sl, err := mtc.SfcContract().LastValidatorID(nil)
	if err != nil {
		mtc.log.Errorf("failed to get the last staker ID: %s", err.Error())
		return 0, err
	}
	return sl.Uint64(), nil
}

// ValidatorsCount returns the number of validators in MetaTech blockchain.
func (mtc *MtcBridge) ValidatorsCount() (uint64, error) {
	// get the value from the contract
	...
	// get the value from the contract
	val, err := mtc.SfcContract().GetEpochValidatorIDs(nil, epoch)
	if err != nil {
		mtc.log.Errorf("failed to get the list of validators; %s", err.Error())
		return 0, err
	}
...
func (mtc *MtcBridge) Validator(valID *big.Int) (*types.Validator, error) {
	// no validator id?
	if valID == nil {
	...
	mtc.log.Debugf("loading validator #%d", valID.Uint64())
	return mtc.validatorById(valID)
}

// validatorById loads details of a validator with the specified ID.
func (mtc *MtcBridge) validatorById(valID *big.Int) (*types.Validator, error) {
	// call for data
	val, err := mtc.SfcContract().GetValidator(nil, valID)
	if err != nil {
		mtc.log.Criticalf("failed to load validator #%d from SFC; %s", valID.Uint64(), err.Error())
		return nil, err
	}

	// any creation record?
	if 0 == val.CreatedTime.Uint64() {
		mtc.log.Errorf("validator #%d has zero created time, assuming empty record", valID.Uint64())
	...
	// keep track of the operation
	mtc.log.Debugf("validator #%d is %s", valID.Uint64(), val.Auth.String())
...
func (mtc *MtcBridge) ValidatorAddress(valID *big.Int) (*common.Address, error) {
	// do we have an address call?
	val, err := mtc.SfcContract().GetValidator(nil, valID)
	if err != nil {
		mtc.log.Error("validator information could not be extracted")
		return nil, err
	}
...
func (mtc *MtcBridge) IsValidator(addr *common.Address) (bool, error) {
	// keep track of the operation
	mtc.log.Debugf("verifying validator address %s", addr.String())

	// try to get the id
	id, err := mtc.SfcContract().GetValidatorID(nil, *addr)
	if err != nil {
		mtc.log.Criticalf("can not check validator at %s; %s", addr.String(), err.Error())
		return false, err
	}
...
func (mtc *MtcBridge) ValidatorByAddress(addr *common.Address) (*types.Validator, error) {
	// no validator id?
	if addr == nil {
	...
	// keep track of the operation
	mtc.log.Debugf("loading validator with address %s", addr.String())

	// try to get the staker id
	id, err := mtc.SfcContract().GetValidatorID(mtc.DefaultCallOpts(), *addr)
	if err != nil {
		mtc.log.Criticalf("can not check validator at %s; %s", addr.String(), err.Error())
		return nil, err
	}
	...
	// do we have the ID?
	if 0 == id.Uint64() {
		mtc.log.Debugf("validator not found for address %s", addr.String())
		return nil, nil
	}
	return mtc.validatorById(id)
}
