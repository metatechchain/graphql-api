...
func (mtc *MtcBridge) Erc165SupportsInterface(address *common.Address, interfaceID [4]byte) (bool, error) {
	// connect the contract
	contract, err := contracts.NewERC165(*address, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC165 contract; %s", err.Error())
		return false, err
	}

	supports, err := contract.SupportsInterface(nil, interfaceID)
	if err != nil {
		mtc.log.Noticef("interface support by ERC165 for contract %s cannot be detected; %s", address.String(), err.Error())
		return false, err
	}
