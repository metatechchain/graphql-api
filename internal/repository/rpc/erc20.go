...
//go:generate tools/abigen.sh --abi ./contracts/abi/wmtc.abi --pkg contracts --type ErcWrappedMtc --out ./contracts/erc20wmtc_token.go

// Erc20Name provides information about the name of the ERC20 token.
func (mtc *MtcBridge) Erc20Name(token *common.Address) (string, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC20 contract; %s", err.Error())
		return "", err
	}
	...
	if err != nil {
		mtc.log.Errorf("ERC20 token %s name not available; %s", token.String(), err.Error())
		return "", err
	}
...
func (mtc *MtcBridge) Erc20Symbol(token *common.Address) (string, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC20 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("ERC20 token %s symbol not available; %s", token.String(), err.Error())
		return "", err
	}
...
func (mtc *MtcBridge) Erc20Decimals(token *common.Address) (int32, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC20 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("ERC20 token %s decimals not available; %s", token.String(), err.Error())
		return 0, nil
	}
...
func (mtc *MtcBridge) Erc20BalanceOf(token *common.Address, owner *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC20 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not ERC20 %s balance for %s; %s", token.String(), owner.String(), err.Error())
		return hexutil.Big{}, err
	}
	...
	if val == nil {
		mtc.log.Errorf("no balance available for ERC20 %s, owner %s", token.String(), owner.String())
		val = new(big.Int)
	}
...
func (mtc *MtcBridge) Erc20Allowance(token *common.Address, owner *common.Address, spender *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC20 contract; %s", err.Error())
	...
	if nil == spender {
		addr := mtc.fMintCfg.mustContractAddress(fMintAddressMinter)
		spender = &addr
	}
	...
	if err != nil {
		mtc.log.Errorf("can not get defi ERC20 %s allowance for %s; %s", token.String(), owner.String(), err.Error())
		return hexutil.Big{}, err
	}
	...
	// return zero even for unknown owners, but let's be sure here
	if val == nil {
		mtc.log.Errorf("no allowance available for ERC20 %s, owner %s", token.String(), owner.String())
		val = new(big.Int)
	}
...
func (mtc *MtcBridge) Erc20TotalSupply(token *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC20 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not get ERC20 %s total supply; %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}

	// make sure we always have a value; at least zero
	if val == nil {
		mtc.log.Errorf("no supply available for ERC20 %s", token.String())
		val = new(big.Int)
	}
