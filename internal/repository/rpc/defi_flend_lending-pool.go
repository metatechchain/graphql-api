...
func (mtc *MtcBridge) FLendGetLendingPool() (*contracts.ILendingPool, error) {
	// get the lending pool contract
	lp, err := contracts.NewILendingPool(mtc.fLendCfg.lendigPoolAddress, mtc.eth)
	if err != nil {
		mtc.log.Errorf("Can not get lending pool contract on address %s; %s", mtc.fLendCfg.lendigPoolAddress.String(), err.Error())
		return nil, err
	}
	return lp, nil
}
...
func (mtc *MtcBridge) FLendGetLendingPoolReserveData(assetAddress *common.Address) (*types.ReserveData, error) {

	// get the lending pool contract
	lp, err := mtc.FLendGetLendingPool()
	if err != nil {
		mtc.log.Errorf("Can not access lending pool %s", err.Error())
		return nil, err
	}

	rd, err := lp.GetReserveData(&bind.CallOpts{}, *assetAddress)
	if err != nil {
		mtc.log.Errorf("Cannot get reserve data for asset %s: %s", assetAddress.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) FLendGetReserveList() ([]common.Address, error) {
	...
	if err != nil {
		mtc.log.Errorf("Cannot get reserves list: %s", err.Error())
		return nil, err
	}
	return rl, nil
}
...
func (mtc *MtcBridge) FLendGetUserAccountData(userAddress *common.Address) (*types.FLendUserAccountData, error) {
	...
	if err != nil {
		mtc.log.Errorf("Cannot get user account data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}

	uc, err := lp.GetUserConfiguration(&bind.CallOpts{}, *userAddress)
	if err != nil {
		mtc.log.Errorf("Cannot get user account configuration data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) FLendGetUserDepositHistory(userAddress *common.Address, assetAddress *common.Address) ([]*types.FLendDeposit, error) {
	// create user filter
	...
	if err != nil {
		mtc.log.Errorf("can not filter lending pool deposit logs: %s", err.Error())
		return nil, err
	}
	...
	for fdi.Next() {
		...
		blk, err := mtc.BlockByHash(&blkHash)
		if err != nil {
			mtc.log.Errorf("fLend block with hash %s was not found: %s", blkHash, err.Error())
			continue
		}
