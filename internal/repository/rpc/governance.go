...
func (mtc *MtcBridge) GovernanceProposalsCount(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not count governance %s proposals; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
// specified by its id.
func (mtc *MtcBridge) GovernanceProposal(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// get the detail
	gp, err := mtc.governanceProposalDetail(gc, gov, id.ToInt())
	if err != nil {
		mtc.log.Errorf("proposal %d not available in %s; %s", id.ToInt().Uint64(), gov.String(), err.Error())
		return nil, err
	}
...
// GovernanceProposal provides a detail of Proposal of a governance contract
// specified by its id.
func (mtc *MtcBridge) governanceProposalDetail(gc *contracts.Governance, govId *common.Address, id *big.Int) (*types.GovernanceProposal, error) {
	...
	ext, err := mtc.GovernanceProposalDetails(&data.ProposalContract)
	if err != nil {
		return nil, err
	}
...
// specified by its id.
func (mtc *MtcBridge) GovernanceProposalState(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposalState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not access governance %s proposal %d state; %s", gov.String(), id.ToInt().Int64(), err.Error())
		return nil, err
	}
...
// specified by its id.
func (mtc *MtcBridge) GovernanceProposalDetails(prop *common.Address) (*govProposalExtended, error) {
	// get the proposal contract
	pp, err := contracts.NewGovernanceProposal(*prop, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance proposal %s; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("governance proposal %s name not available; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("governance proposal %s description not available; %s", prop.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) GovernanceOptionState(gov *common.Address, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	gs, err := mtc.GovernanceOptionStateById(gc, propId, optId)
	if err != nil {
		mtc.log.Errorf("governance %s proposal #%d state #%d not available; %s",
...
func (mtc *MtcBridge) GovernanceOptionStates(gov *common.Address, propId *hexutil.Big, optRange int) ([]*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	for i := 0; i < optRange; i++ {
		...
		res[i], err = mtc.GovernanceOptionStateById(gc, propId, (*hexutil.Big)(big.NewInt(int64(i))))
		if err != nil {
			mtc.log.Errorf("unknown option #%d on governance %s; %s", i, gov.String(), err.Error())
			break
		}
	}
	return res, nil
...
func (mtc *MtcBridge) GovernanceOptionStateById(gc *contracts.Governance, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the state
...
func (mtc *MtcBridge) GovernanceVote(
	gov *common.Address,
	propId *hexutil.Big,
	from *common.Address,
	...
	if err != nil {
		mtc.log.Errorf("can not access vote of %s on governance %s; %s", from.String(), gov.String(), err.Error())
		return nil, err
	}
...
func (mtc *MtcBridge) GovernanceProposalsBy(gov *common.Address) ([]*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// log what we do
	mtc.log.Noticef("loading %d proposals of %s", maxProposalId.Uint64(), gov.String())
	...
	for i := maxProposalId.Int64(); i > 0; i-- {
		...
		gp, err := mtc.governanceProposalDetail(gc, gov, big.NewInt(i))
		if err != nil {
			mtc.log.Errorf("can not access governance %s proposal %d; %s", gov.String(), i, err.Error())
			return nil, err
		}

		// keep the proposal in the list
		mtc.log.Noticef("found proposal #%d on %s", gp.Id.ToInt().Uint64(), gov.String())
		result = append(result, gp)
	}

	// log what we do
	mtc.log.Debugf("%d proposals of %s loaded", maxProposalId.Uint64(), gov.String())
	return result, nil
}

// GovernanceProposalFee returns the fee payable for a new proposal
// in given Governance contract context.
func (mtc *MtcBridge) GovernanceProposalFee(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		mtc.log.Errorf("governance %s fee not available; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (mtc *MtcBridge) GovernanceTotalWeight(ge *common.Address) (*hexutil.Big, error) {
	// get the contract
	goe, err := contracts.NewGovernable(*ge, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not access governable adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}

	// get the total available voting weight
	w, err := goe.GetTotalWeight(mtc.DefaultCallOpts())
	if err != nil {
		mtc.log.Errorf("total weight not available on adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}
