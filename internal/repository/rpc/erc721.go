...
func (mtc *MtcBridge) Erc721Name(token *common.Address) (string, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
		return "", err
	}
	...
	if err != nil {
		mtc.log.Errorf("ERC721 token %s name not available; %s", token.String(), err.Error())
		return "", err
	}
...
func (mtc *MtcBridge) Erc721Symbol(token *common.Address) (string, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("ERC721 token %s symbol not available; %s", token.String(), err.Error())
		return "", err
	}
...
func (mtc *MtcBridge) Erc721BalanceOf(token *common.Address, owner *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not ERC721 %s balance for %s; %s", token.String(), owner.String(), err.Error())
		return hexutil.Big{}, err
	}
	...
	if val == nil {
		mtc.log.Errorf("no balance available for ERC721 %s, owner %s", token.String(), owner.String())
		val = new(big.Int)
	}
...
func (mtc *MtcBridge) Erc721TotalSupply(token *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not get ERC721 %s total supply; %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}

	// make sure we always have a value; at least zero
	if val == nil {
		mtc.log.Errorf("no supply available for ERC721 %s", token.String())
		val = new(big.Int)
	}
...
func (mtc *MtcBridge) Erc721TokenURI(token *common.Address, tokenId *big.Int) (string, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("ERC721 token %s/%s URI not available; %s", token.String(), tokenId.String(), err.Error())
		return "", err
	}
...
func (mtc *MtcBridge) Erc721OwnerOf(token *common.Address, tokenId *big.Int) (common.Address, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not get ERC721 %s owner of %s; %s", token.String(), tokenId.String(), err.Error())
		return common.Address{}, err
	}
...
func (mtc *MtcBridge) Erc721GetApproved(token *common.Address, tokenId *big.Int) (common.Address, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not get ERC721 %s approved operator of %s; %s", token.String(), tokenId.String(), err.Error())
		return common.Address{}, err
	}
...
func (mtc *MtcBridge) Erc721IsApprovedForAll(token *common.Address, owner *common.Address, operator *common.Address) (bool, error) {
	// connect the contract
	contract, err := contracts.NewERC721(*token, mtc.eth)
	if err != nil {
		mtc.log.Errorf("can not contact ERC721 contract; %s", err.Error())
	...
	if err != nil {
		mtc.log.Errorf("can not get ERC721 %s approved-for-all status for owner %s and operator %s; %s", token.String(), owner.String(), operator.String(), err.Error())
		return false, err
	}
