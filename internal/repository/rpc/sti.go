...
func (mtc *MtcBridge) StakerInfo(id *hexutil.Big) (*types.StakerInfo, error) {
	if id == nil {
		return nil, fmt.Errorf("staker ID not given")
	}

	// keep track of the operation
	mtc.log.Debugf("loading staker information for staker #%d", id.ToInt().Uint64())

	// instantiate the contract and display its name
	contract, err := contracts.NewStakerInfoContract(mtc.sfcConfig.StiContract, mtc.eth)
	if err != nil {
		mtc.log.Criticalf("failed to instantiate STI contract: %v", err)
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("failed to get the staker #%d information: %v", id.ToInt().Uint64(), err)
		return nil, err
	}

	// var url string
	if len(stUrl) == 0 {
		mtc.log.Debugf("no information for staker #%d", id.ToInt().Uint64())
		return nil, nil
	}

	// try to download JSON for the info
	return mtc.downloadStakerInfo(stUrl)
}

// downloadStakerInfo tries to download staker information from the given URL address.
func (mtc *MtcBridge) downloadStakerInfo(stUrl string) (*types.StakerInfo, error) {
	// log what we are about to do
	mtc.log.Debugf("downloading staker info address [%s]", stUrl)
	...
	if err != nil {
		mtc.log.Errorf("can not request given staker info at [%s]; %s", stUrl, err.Error())
		return nil, err
	}

	// be honest, set agent
	req.Header.Set("User-Agent", "MetaTech GraphQL API Server")

	// process the request
	res, err := cl.Do(req)
	if err != nil {
		mtc.log.Errorf("can not download staker info from [%s]; %s", stUrl, err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("can not read staker info response from [%s]; %s", stUrl, err.Error())
		return nil, err
	}
	...
	if err != nil {
		mtc.log.Errorf("can not decode staker info [%s]; %s", stUrl, err.Error())
		mtc.log.Debugf("Received: %s", body)
		return nil, err
	}

	// do we have anything?
	if !mtc.isValidStakerInfo(&info) {
		mtc.log.Errorf("invalid staker info content [%s]", stUrl)
		return nil, err
	}

	mtc.log.Debugf("found staker [%s]", *info.Name)
	return &info, nil
}

// isValidStakerInfo check if the staker information is valid and can be used.
func (mtc *MtcBridge) isValidStakerInfo(info *types.StakerInfo) bool {
	// name must be available
	if nil == info.Name || 0 == len(*info.Name) || !stiNameCheckRegex.Match([]byte(*info.Name)) {
		mtc.log.Error("staker name not valid")
		return false
	}

	// check the logo URL
	if !isValidStakerInfoUrl(info.LogoUrl, true) {
		mtc.log.Error("staker logo URL not valid")
		return false
	}

	// check the website
	if !isValidStakerInfoUrl(info.Website, false) {
		mtc.log.Error("staker website URL not valid")
		return false
	}

	// check the contact URL
	if !isValidStakerInfoUrl(info.Contact, false) {
		mtc.log.Error("staker contact URL not valid")
		return false
	}
	return true
}
