...
func (mtc *MtcBridge) MustBlockHeight() *big.Int {
	var val hexutil.Big
	if err := mtc.rpc.Call(&val, "mtc_blockNumber"); err != nil {
		mtc.log.Errorf("failed block height check; %s", err.Error())
		return nil
	}
	return val.ToInt()
}

// BlockHeight returns the current block height of the MetaTech blockchain.
func (mtc *MtcBridge) BlockHeight() (*hexutil.Big, error) {
	// keep track of the operation
	mtc.log.Debugf("checking current block height")

	// call for data
	var height hexutil.Big
	err := mtc.rpc.Call(&height, "mtc_blockNumber")
	if err != nil {
		mtc.log.Error("block height could not be obtained")
		return nil, err
	}

	// inform and return
	mtc.log.Debugf("current block height is %s", height.String())
	return &height, nil
}
...
func (mtc *MtcBridge) Block(numTag *string) (*types.Block, error) {
	// keep track of the operation
	mtc.log.Debugf("loading details of block num/tag %s", *numTag)

	// call for data
	var block types.Block
	err := mtc.rpc.Call(&block, "mtc_getBlockByNumber", numTag, false)
	if err != nil {
		mtc.log.Error("block could not be extracted")
		return nil, err
	}
	...
	if uint64(block.Number) == 0 && block.Hash.Big().Cmp(big.NewInt(0)) == 0 {
		mtc.log.Debugf("block [%s] not found", *numTag)
		return nil, fmt.Errorf("block not found")
	}

	// keep track of the operation
	mtc.log.Debugf("block #%d found at mark %s",
...
func (mtc *MtcBridge) BlockByHash(hash *string) (*types.Block, error) {
	// keep track of the operation
	mtc.log.Debugf("loading details of block %s", *hash)

	// call for data
	var block types.Block
	err := mtc.rpc.Call(&block, "mtc_getBlockByHash", hash, false)
	if err != nil {
		mtc.log.Error("block could not be extracted")
		return nil, err
	}
	...
	if uint64(block.Number) == 0 {
		mtc.log.Debugf("block [%s] not found", *hash)
		return nil, fmt.Errorf("block not found")
	}

	// inform and return
	mtc.log.Debugf("block #%d found at mark %s by hash %s",
