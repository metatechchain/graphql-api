...
// burnDispatcher implements dispatcher of new native MTC burns on the blockchain.
...
func (bud *burnDispatcher) process(tx *eventTrx, burn *types.MtcBurn) *types.MtcBurn {
	// no previous burn to check against? make a new record
	if burn == nil {
		return &types.MtcBurn{
	...
	if tx.blk.Number != burn.BlockNumber {
		...
		log.Debugf("collected block burn of %.4f MTC at #%d", val, burn.BlockNumber)

		if err := repo.StoreMtcBurn(burn); err != nil {
...
// burnedFee calculates the amount of burned MTCs from the transaction fee.
